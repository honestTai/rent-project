...
const platformMutationPatterns = [
  /^\/api\/platform\/config\/save$/,
  /^\/api\/platform\/notify-channel\/save$/,
  /^\/api\/platform\/schedule-task\/save$/,
  /^\/api\/platform\/internal\/config\/save$/,
  /^\/api\/platform\/rbac\/roles\/save$/,
  /^\/api\/platform\/rbac\/roles\/permissions\/save$/,
  /^\/api\/platform\/rbac\/users\/roles\/save$/,
  /^\/api\/user\/(addUser|updateUser|deleteUser|resetPassword)$/,
];

const resolveMutationText = (path: string) => {
  if (/delete/i.test(path)) return '删除后数据可能无法恢复，是否继续？';
  if (/(^|\/)(add|create|save)/i.test(path)) return '即将新增或保存数据，是否继续？';
  if (/update|reset|roles\/permissions|users\/roles/i.test(path)) return '即将修改数据，是否继续？';
...
export const shouldConfirmMutationRequest = (method?: string, url?: string) => {
  ...
  return requestMethod === 'post' && platformMutationPatterns.some((pattern) => pattern.test(path));
};
