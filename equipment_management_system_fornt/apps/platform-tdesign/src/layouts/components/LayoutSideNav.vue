...
const flattenPlatformRoot = (routers: Array<MenuRoute>) => {
  if (routers.length !== 1) return routers;
  const [root] = routers;
  if (root.path !== '/platform' || !root.children?.length) return routers;
  return root.children.map((child) => ({
    ...child,
    path: child.path.startsWith('/') ? child.path : `${root.path}/${child.path}`,
  }));
};
...
const sideMenu = computed(() => {
  ...
  return flattenPlatformRoot(newMenuRouters);
});
</script>
